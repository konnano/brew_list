  ...
  open my $BREW1,'<',"$dir.rb" or die " Info_1 $!\n";
   while(my $data=<$BREW1>){
    ...
    if( $data =~ /^\s*on_linux\s*do.*/ ){
     $i = 2; next;
    }elsif( $data !~ /^\s*end/ and $data =~ /.*keg_only.*/ and $i == 2 ){
     $tap{"${name}keg_Linux"} = 1; next;
    }elsif( $data =~ /^\s*end/ and $i == 2 ){
     $i = 0; next;
    }

     if( $data =~ /.*keg_only.*macos.*/ ){
      $tap{"${name}keg"} = 1; next;
     }
     $tap{"${name}keg_Linux"} = $tap{"${name}keg"} = 1 if $data =~ /.*keg_only.*/;
    $tap{"${name}un_Linux"} = 1  if $data =~ /.*depends_on\s*:macos.*/;
   }
  close $BREW1;
 }
untie %tap;
