...
unless( $ENV{'HOMEBREW_NO_INSTALL_FROM_API'} ){
 ...
 if( $re->{'MAC'} ){
   $re->{'CLANG'} = `/usr/bin/clang --version|sed -E '/Apple/!d;s/.+clang-([^.]+).+/\\1/;q'` || 0 unless $ARGV[0];
    ...
    while(my $data=<$BREW>){ last if $data =~ /^\s*def\s+install/;
      if( $data =~ /^\s*bottle\s+do/ ){
       $KIN = $bot = 1; next;
        ...
        if( $data =~ /.*,\s+all:/ ){
         $tap{"${name}15.0M1"}= $tap{"${name}15.0"} = $tap{"${name}14.0M1"}= $tap{"${name}14.0"} =
         $tap{"${name}13.0M1"}= $tap{"${name}13.0"} = $tap{"${name}12.0M1"}= $tap{"${name}12.0"} =
         $tap{"${name}11.0M1"}= $tap{"${name}11.0"} = $tap{"${name}10.15"} = $tap{"${name}10.14"}=
         $tap{"${name}10.13"} = $tap{"${name}10.12"}= $tap{"${name}10.11"} = $tap{"${name}x86_64_linux"} = 1;
          $KIN = 0; next;
        }
         if( $re->{'LIN'} ){
