 ...
 unless( $ARGV[0] ){
  ...
  my $CPU = $UNAME eq 'x86_64' ? 'intel' : 'arm';
   ...
   for my $ls2(sort readdir $dir2){ next if index($ls2,'.') == 0;
    my( $name,$u2,$ato,@ui,@ta ) = ( substr($ls2,0,-4),'' );
     $ta[0] = $ta[1] = 1; push @CASK,$name; $ACA .= "$name \\\n";
    ...
    open my $CASK,'<',"$ENV{'HOME'}/.BREW_LIST/cparse/$ls2" or die" 2 cparse $!\n";
     while(my $data = <$CASK>){
      unless( $ui[9] ){
       ...
       if( $ta[1] and $data =~ /"disable_date":"([^"]+)"/ and $TIME gt $1 ){ $tap{"${name}c_disable"} = 1; $ta[1] = 0; }
      }
 ...
 rmdir "$ENV{'HOME'}/.BREW_LIST/17";
  ...
  if( $re->{'MAC'} ){
   for my $dir2( @CA ){ my( $ver,@ta );
    ...
    for(0..4){ $ta[$_] = 1 }
    if( $dirs eq 'homebrew-cask-fonts' ){ push @CASK,$name;
     $tap{"${name}mfont"} = 1;
      $tap{"${name}cask"} = $dir2;
