...
my( $re,$cache,$OS_Version,$OS_Version2,%MAC_OS,%Mac_OS,$Xcode,%HAN,@BREW,@CASK,$NAM,$ACA,$FON,@font,@cas ); ###
...
unless( $ENV{'HOMEBREW_NO_INSTALL_FROM_API'} ){
  ...
  for my $ls1(sort readdir $dir1){ next if index($ls1,'.') == 0;
   ...
   my( $name,$u2,$b2,%HA1,%HA2,%LHA1,%LHA2,@ui,@ta,@lu,@sp ) = ( substr($ls1,0,-4),0 );
    for(0..11){ $ta[$_] = 1 } push @BREW,$name; $NA{$name}++;
     $tap{"${name}core"} = "$MY_HOME/homebrew--core/$name";
   open my $BREW,"$ENV{'HOME'}/.BREW_LIST/parse/$ls1" or die" 2 parse $!\n";
    while(my $data = <$BREW> ){
     unless( $ui[22] ){

       if( $ta[0] and $data =~ /^,"desc":"([^"]+)"/ ){ $tap{"${name}f_desc"} = $1; $ta[0] = 0 }
   ...
   for my $ls2(sort readdir $dir2){ next if index($ls2,'.') == 0;
    my( $name,$u2,$ato,$url,@ui,@ta ) = ( substr($ls2,0,-4),'' );
     for(0..2){ $ta[$_] = 1 } push @CASK,$name;
      $tap{"${name}cask"} = "$MY_HOME/homebrew--cask/$name";
     ...
     while(my $data = <$CASK>){
      unless( $ui[9] ){
       ...
       if( index($data,'{"font":[') == 0 ){				push @font,"$name\n";
        if( $re->{'MAC'} ){ $tap{"${name}mfont"} = 1 }else { $tap{"${name}lfont"} = 1 }
 ...
 rmdir "$ENV{'HOME'}/.BREW_LIST/17";
  ...
  my( $dir,$cask ) = @_;
   opendir my $DIR,$dir or die " DIR $!\n";
    for my $an( sort readdir $DIR ){ next if index($an,'.') == 0;
      next if $an =~ /homebrew$|homebrew-core$|homebrew-cask$|homebrew-bundle$|homebrew-services$|
      ...
      -d "$dir/$an" ? Dirs_1( "$dir/$an",$cask) :( $cask and $an =~ /\.rb$/ ) ?
  ...
  for my $dir1( @BR ){
   ...
   while( my $data = <$BR1> ){
    if( $ta[0] and $data =~ /^\s*desc\s*"([^"]+)"/ ){ $tap{"${name}f_desc"} = $1; $ta[0] = 0 }
  ...
  if( $re->{'MAC'} ){
   for my $dir2( @CA ){ my( $ver,@ta );
    ...
    if( $dirs eq 'homebrew-cask-fonts' ){ push @CASK,$name;		push @cas,"$name\n";		###
     ...
     open my $BR2,'<',$dir2 or die " tie Info_2 $!\n";
      while(my $data=<$BR2>){
      if( $ta[0] ){
       if( $ta[0] and $data =~ /^\s*version\s+"([^"]+)"/ ){ $ver = $1; $ta[0] = 0 }
   ...
   for my $dir3( @CA ){
    my( $dirs,$name,@ta ) = $dir3 =~ m|.+/(homebrew-[^/]+)/(?:[^/]+/)*(.+)\.rb$|;
     for(0..2){ $ta[$_] = 1 }
    if( $dirs eq 'homebrew-linux-fonts' ){ push @CASK,$name;		push @cas,"$name\n";		###
...
unless( $ENV{'HOMEBREW_NO_INSTALL_FROM_API'} or $ARGV[0] ){				###
 my $sort = '';
